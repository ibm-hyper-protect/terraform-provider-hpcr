...
func TestTgzEncryptedResource_Metadata(t *testing.T) {
	r := NewTgzEncryptedResource()
	...
	if resp.TypeName != "hpcr_tgz_encrypted" {
		t.Errorf("Expected TypeName to be 'hpcr_tgz_encrypted', got '%s'", resp.TypeName)
	}
}

func TestTgzEncryptedResource_Schema(t *testing.T) {
	r := NewTgzEncryptedResource()
	...
	requiredAttrs := []string{"id", "folder", "cert", "platform", "rendered", "sha256_in", "sha256_out"}
	...
	// Verify cert and platform are optional
...
func TestNewTgzEncryptedResource(t *testing.T) {
	r := NewTgzEncryptedResource()
	if r == nil {
		t.Fatal("NewTgzEncryptedResource should not return nil")
	}
...
func TestTgzEncryptedResource_Delete(t *testing.T) {
	r := &TgzEncryptedResource{}
