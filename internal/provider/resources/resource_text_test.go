...
func TestTextResource_Metadata(t *testing.T) {
	r := NewTextResource()
	...
	if resp.TypeName != "hpcr_text" {
		t.Errorf("Expected TypeName to be 'hpcr_text', got '%s'", resp.TypeName)
	}
}

func TestTextResource_Schema(t *testing.T) {
	r := NewTextResource()
	...
	requiredAttrs := []string{"id", "text", "rendered", "sha256_in", "sha256_out"}
...
func TestNewTextResource(t *testing.T) {
	r := NewTextResource()
	if r == nil {
		t.Fatal("NewTextResource should not return nil")
	}
...
func TestTextResource_Delete(t *testing.T) {
	r := &TextResource{}
