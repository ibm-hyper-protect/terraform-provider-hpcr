...
func TestJSONEncryptedResource_Metadata(t *testing.T) {
	r := NewJSONEncryptedResource()
	...
	if resp.TypeName != "hpcr_json_encrypted" {
		t.Errorf("Expected TypeName to be 'hpcr_json_encrypted', got '%s'", resp.TypeName)
	}
}

func TestJSONEncryptedResource_Schema(t *testing.T) {
	r := NewJSONEncryptedResource()
	...
	requiredAttrs := []string{"id", "json", "cert", "platform", "rendered", "sha256_in", "sha256_out"}
...
func TestNewJSONEncryptedResource(t *testing.T) {
	r := NewJSONEncryptedResource()
	if r == nil {
		t.Fatal("NewJSONEncryptedResource should not return nil")
	}
...
func TestJSONEncryptedResource_Delete(t *testing.T) {
	r := &JSONEncryptedResource{}
